...
interface BasicCommands {
     ...
     * Request for authentication in a password-protected Redis server. Redis can be instructed to require a password 
     before allowing clients to execute commands. This is done using the requirepass directive in the configuration file.
     ...
     * Instead usually BGSAVE is used. However in case of issues preventing Redis to create the background 
     * saving child (for instance errors in the fork(2) system call), the SAVE command can be a good last 
    ...
    /**
     * Save the DB in background. The OK code is immediately returned. Redis forks, the parent continues 
     * to serve the clients, the child saves the DB on disk then exits. A client may be able to check if 
     * the operation succeeded using the LASTSAVE command.
     * @return ok
     */
    string bgsave();

    /**
     * Instruct Redis to start an Append Only File rewrite process. The rewrite will create a small optimized
     * version of the current Append Only File If BGREWRITEAOF fails, no data gets lost as the old AOF will
     * be untouched. The rewrite will be only triggered by Redis if there is not already a background process
     * doing persistence. Specifically: If a Redis child is creating a snapshot on disk, the AOF rewrite is 
     * scheduled but not started until the saving child producing the RDB file terminates. In this case the 
     * BGREWRITEAOF will still return an OK code, but with an appropriate message. You can check if an AOF 
     * rewrite is scheduled looking at the INFO command as of Redis 2.6.
