...
abstract class PipelineBase : Queable, RedisPipeline, RedisPipeline {
    ...
    override
    Response!(Long) lpushx(string key, string[] bytes...) {
    ...
    override
    Response!(Boolean) setbit(string key, long offset, string value) {
        getClient(key).setbit(key, offset, value);
        return getResponse(BuilderFactory.BOOLEAN);
    }
    ...
    override
    Response!(string) xadd(string key, string id, Map!(string, string) hash){
        return xadd(key, id, hash, Long.MAX_VALUE, true);
    }
    ...
    override
    Response!(string) xadd(string key, string id, Map!(string, string) hash, long maxLen, bool approximateLength){
        getClient(key).xadd(key, id, hash, maxLen, approximateLength);
        return getResponse(BuilderFactory.BYTE_ARRAY);        
    }
    ...
    override
    Response!(List!(string)) xrange(string key, string start, string end, int count){
        getClient(key).xrange(key, start, end, count);
        return getResponse(BuilderFactory.BYTE_ARRAY_LIST);            
    }
    ...
    override
    Response!(List!(string)) xrevrange(string key, string end, string start, int count){
        getClient(key).xrevrange(key, start, end, count);
        return getResponse(BuilderFactory.BYTE_ARRAY_LIST);                
    }
    ...
    override
    Response!(Long) xack(string key, string group,  string[] ids...){
        getClient(key).xack(key, group, ids);
        return getResponse(BuilderFactory.LONG);                    
    }
    ...
    override
    Response!(string) xgroupCreate(string key, string groupname, string id, bool makeStream){
        getClient(key).xgroupCreate(key, groupname, id, makeStream);
        return getResponse(BuilderFactory.STRING);    
    }
    ...
    override
    Response!(string) xgroupSetID(string key, string groupname, string id){
        getClient(key).xgroupSetID(key, groupname, id);
        return getResponse(BuilderFactory.STRING);    
    }
    ...
    override
    Response!(List!(StreamPendingEntry)) xpending(string key, string groupname, string start, string end, int count, string consumername){
        getClient(key).xpending(key, groupname, start, end, count, consumername);
        return getResponse(BuilderFactory.STREAM_PENDING_ENTRY_LIST);            
    }
    ...
    override
    Response!(Long) xdel(string key, string[] ids...){
        getClient(key).xdel(key, ids);
        return getResponse(BuilderFactory.LONG);            
    }
    ...
    override
    Response!(List!(string)) xclaim(string key, string group, string consumername, long minIdleTime, 
    ...
    override
    Response!(Object) sendCommand(ProtocolCommand cmd, string[] args...){
        string key = args.length > 0 ? args[0] : cast(string)(cmd.to!string());
        getClient(key).sendCommand(cmd, args);
        return getResponse(BuilderFactory.OBJECT);
    }
}
